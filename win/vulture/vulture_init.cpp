...
void vulture_show_darkarts_intro(void)
{
  ...
  decoder = THEORAPLAY_startDecodeFile(fname.c_str(), 100, THEORAPLAY_VIDFMT_IYUV);
  if (!decoder)
  {
  ...
  while (!quit && THEORAPLAY_isDecoding(decoder))
  {
      ...
      if (video && (video->playms <= now))
      {
          ...
          else
          {
              ...
              dstrect.w = vulture_screen->w;
              dstrect.h = vulture_screen->h;
      ...
      memset(&event, 0, sizeof(event)); // reset event
      //SDL_Delay(100); // give some cpu XXX strange pollevent valgrind uninit issue
