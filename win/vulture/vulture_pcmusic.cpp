...
#define INSTRUMENT_FILE V_MUSIC_DIRECTORY "/instruments.xm"
...
static void vulture_pcmusic_done( void )
{
...
void vulture_pcmusic_init( void )
{
	if (!vulture_opts.play_effects) return;

	atexit( vulture_pcmusic_done );
	if (!pcmusic_load_instruments) vulture_write_log(V_LOG_ERROR, __FILE__, __LINE__, "Unable to load instruments");
}
...
void pc_speaker(struct obj *instr, char *tune)
{
	...
	if (!vulture_opts.play_effects || !pcmusic_load_instruments) return;
